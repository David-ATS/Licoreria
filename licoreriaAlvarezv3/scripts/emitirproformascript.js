...
function hacertabla(){
	...
	for(var i=0;i<lista.length;i++){
		...
		tabla+='<tr>'+
				...
				'<td>'+
					'<button name="Eliminar" onclick="eliminar(\''+lista[i].codigo+'\')"><i class="material-icons">delete</i></button>'+
					'<button name="Editar" id="btneditar'+i+'" onclick="editar(\''+lista[i].codigo+'\','+i+')"><i class="material-icons">edit</i></button>'+
					'<button name="Guardar" id="btnsave'+i+'" onclick="save(\''+lista[i].codigo+'\','+i+')" disabled="true"><i class="material-icons">save</i></button>'+
					'</td>'+
			   '</tr>'
			   '</table>'
			   '</body>';
			...
			if(resultado==false){
				...
				if(resultado=="ok"){
				 ...
				 insertardetalle();
					...
					alert("error al insertar proforma");
				}
	...
	$.get("getProforma.php?i="+serie,{json:json},function(resultado){
			...
			if(resultado==false){
				...
				if(resultado=="ok"){
				 ...
				 savepdf();
				 alert("La proforma se emitio con exito");
				 location.reload();
				}else{
					alert("error al insertar detalle de proforma");
				}
			}
	...
	if(lista.length==0){
		...
		var proforma='<div id="contenedor">'+
					 '<div id="proforma">'+
						...
						'<tr><td><label>Fecha: </label><div id="fecha">'+anio+'-'+mes+'-'+dia+'</div></td><td><label>Hora de emision: </label><div id="hora">'+hora+':'+minuto+':'+segundo+'</div></td></tr>'+
						'</table>'+
						'</div>'+
						'<br>'+
						'<div id="tabla">'+
								//	   <!-- 0       1          2         3        4       5 -->
					//	<!--//console.log(codigo , stock, descripcion, precio, cantidad, total);-->
							'<table name="tablaemer" width="100%" border="1">'+
							'<tbody>'+
							  '<thead>'+
							  '<tr>'+
								'<td>CODIGO</td>'+
								'<td>DESCRIPCI&OtildeN</td>'+
								'<td>CANTIDAD</td>'+
								'<td>P.U</td>'+
								'<td>MONTO</td>'+
							  '</tr>'+
	...
	var descripcion = document.getElementById("txtproducto").value;
		...
		xhttp.onreadystatechange = function(){
				if(this.readyState==4 && this.status==200){
					...
					var tabla=			
							'<table name="tabla1" width="444" border="1" cellpadding="5" align="left">'+
								 '<tbody>'+	
								  '<thead>'+
								  	'<td width="87">ID</td>'+
					...
					for(var item of json){
					 ...
					 tabla+=			 
					 		'<tr>'+
								...
								'<td><button name="Agregar" onclick="agregar('+i+')"><i class="material-icons">add_shopping_cart</i></button></td>'+
							 '</tr>';
						i++;		 
					}
					tabla+="</tbody></table>";
